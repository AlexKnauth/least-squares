...
define-simple-macro
  defmatrix id:id [[e:expr ...] ...]
  define id
    matrix [[e ...] ...]

define (linear-least-squares points)
  define-simple-macro ∑[expr:expr ...] #:with [x-id y-id] (syntax-local-introduce #'[xi yi])
...
define (exponential-least-squares/logy points)
  define points/logy
    for/list ([p (in-list points)])
      ...
      list x log(y)
  match-define mx+b[m b]
    linear-least-squares points/logy
  ;; e^(mx+b) = e^b*e^mx
  c*e^ax exp(b) m
  ...
  test-case "best-polynomial"
    for ([n (in-range 1 15)])
      ...
      for ([p (in-list ps)])
        ...
        check-equal? f(x) y
  check-equal?
    exponential-least-squares/logy '([-2 1/4] [-1 1/2] [0 1.0] [1 2] [2 4])
    c*e^ax 1.0 log(2)
