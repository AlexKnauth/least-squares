...
define (my-plot . args)
  plot
    list* axes() tick-grid()
      filter-not(void? flatten(args))

define (plot-linear-least-squares ps)
  define f
    linear-least-squares(ps)
  my-plot
    function f #:label function-struct->string(f)
    points(ps)

define (plot-quadratic-least-squares ps)
  define f
    quadratic-least-squares(ps)
  my-plot
    function f #:label function-struct->string(f)
    points(ps)
...
define (plot-exponential-least-squares/logy ps)
  define f
    exponential-least-squares/logy(ps)
  my-plot
    function f #:label function-struct->string(f)
    points(ps)
...
define exact-random(n)
  ...
  {sgn * {random(n) + inexact->exact(random())}}
...
define (plot-linear-least-squares/random n)
  define ps
    random-points(n 10)
  my-plot
    function linear-least-squares(ps) -10 10
    points(ps)
...
define (plot-best-polynomial/random n #:both? [both? #f])
  ...
  my-plot rect axes()
    when both?
      inverse best-polynomial[(map reverse ps)] -10 10 #:color "gray"
